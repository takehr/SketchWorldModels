...
rasterizer = Rasterizer(num_control_points, num_strokes, device).to(device)
...
for episode in range(seed_episodes, all_episodes):
    ...
    for update_step in range(collect_interval):
        ...
        # 観測と報酬の予測誤差を計算
        obs_loss = 0.5 * 10 * clip_loss(recon_observations.view(-1, 3, 224, 224), observations.view(-1, 3, 224, 224)[batch_size:])#F.mse_loss(recon_observations.view(-1, 3, 224, 224), observations.view(-1, 3, 224, 224)[batch_size:])#
